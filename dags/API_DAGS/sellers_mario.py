...
def extract_api_to_gcs():
    ...
    gcs_object = f'Sellers_API_Mario/data_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
...
with DAG(
    ...
    description='Extract sellers CSV data from API to GCS to BigQuery',
    schedule_interval=timedelta(hours=1),
    catchup=False,
    tags=['sellers', 'api', 'gcs', 'bigquery', 'csv']
) as dag:
    ...
    load_to_bq = GCSToBigQueryOperator(
        ...
        source_objects=['Sellers_API_Mario/*.csv'],
        destination_project_dataset_table='project_landing.sellers_mario',
        source_format='CSV',
        autodetect=True,
